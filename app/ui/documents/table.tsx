...
import { deleteFile, getFiles } from "@/app/lib/actions";
...
export default function UploadedDocumentsTable({
  ...
  documents: documents[];
    ...
    function parseFileClick(fileName:string) {
        redirect('/dashboard/mydocuments/parsefile?f='+fileName);
    }
  ...
  return (
    ...
    <ProcessingDialog message={processingMessage} showProgress={showProgress} />
      ...
      <Label></Label>
      <Link href='/dashboard/mydocuments/upload'><Button icon={<AddCircle24Regular />}>Upload New Document</Button></Link>
      <Label></Label>
        ...
        <TableBody>
          {allDocuments?.map((document) => (
            <TableRow key={document.name}>
              ...
              <TableCell>
                <TableCellLayout>
                  <Link href={`/dashboard/mydocuments/summarize?f=${document.name}`}>
              ...
              <TableCell >
                <TableCellLayout className={styles.buttonwrapper}>
                    ...
                    <Link href={`/dashboard/search?f=${document.name}`}>
